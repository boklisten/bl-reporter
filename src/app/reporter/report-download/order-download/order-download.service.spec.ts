...
describe('OrderDownloadService', () => {
  ...
  it('should call api with correct query based on filter', (done) => {
    ...
    service.getOrdersByFilter(filter).then((returnedOrders) => {
      ...
      expect(args[0])
        .toContain('creationTime=>010120000000');
      
      expect(args[0])
        .toContain('creationTime=<010120010000');

      expect(args[0])
        .toContain('byCustomer=true');

      expect(args[0])
        .toContain('placed=true');
  ...
  it('should call api with correct query if orderItemNotDelivered flag is set in filter', (done) => {
    ...
    service.getOrdersByFilter(filter).then((returnedOrders) => {
      ...
      expect(args[0].indexOf('orderItems.handout') > -1)
        .toBeFalsy();
      
      expect(args[0].indexOf('orderItems.delivered') > -1)
        .toBeFalsy();
  ...
  describe('#filterOrderItems', () => {
    it('should only include orderItems with type defined by filter', () => {
      ...
      expect(service.filterOrderItems(filter, order))
    ...
    describe('when filter flag orderItemNotHandedOut is set', () => {
      it('should not include orderItems with movedToOrder', () => {
        ...
        expect(service.filterOrderItems(filter, order as Order))
          .toEqual([])
      })
      ...
      let expectedOutput = [
        {
          id: orders[1].id,
          orderItems: [
            orders[1].orderItems[0]
          ]
        }
      ]

      expect(service.filterOrders(filter, orders))
        .toEqual(expectedOutput);
